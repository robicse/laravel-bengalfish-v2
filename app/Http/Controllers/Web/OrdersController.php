...
class OrdersController extends Controller
{
   ...
   session(['guest_checkout' => 1]);
	...
	public function checkout(Request $request){
		...
		if(count($result['cart'])==0){
            ...
            if(!empty(auth()->guard('customer')->user()->id)){
                ...
                $record = DB::table('user_to_address')->where('user_id', $customers_id)->first();
                //$address_id = auth()->guard('customer')->user()->customers_default_address_id;
                if(!empty($record))
                {
                    $address = $this->shipping->getShippingAddress($record->address_book_id);
                    if(!empty($address)){
                        $address = $address[0];
	...
	public function place_order(Request $request){
		    //dd($request->all());
		$payment_details = $this->order->place_order($request);
        //return redirect('/checkout/pay');
		if($payment_details['payment_status']=='success'){
            $orders_id = session('order_id');
            ...
            if($payment_details['payment_method'] == 'SSLCOMMERZ')
            {
                //echo 'okk';exit;
