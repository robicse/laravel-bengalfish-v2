...
class CustomersController extends Controller
{
	...
	public function processLogin(Request $request){
		...
		$result = array();
            ...
            if (is_numeric($request->email)){
                //echo 'num';
                $customerInfo = array("phone" => $request->email, "password" => $request->password);
            }else{
                //echo 'email';
  ...
  public function handleSocialLoginCallback($social){
    ...
    public function processPasswordPhone(Request $request){
        $title = array('pageTitle' => Lang::get("website.Forgot Password"));
        ...
        //check email exist
        $existUser = $this->customer->ExistUserPhone($phone);
        if(count($existUser)>0){
            ...
            return redirect('forgotPassword')->with('error', Lang::get("website.Phone number does not exist"));
        }
	...
	public function signupProcess(Request $request){
        ...
        if($validator->fails()){
            ...
            if($res['email_or_phone'] == "true"){
                return redirect('/login')->withInput($request->input())->with('error', Lang::get("website.Email OR Phone already exist"));
            }else{
                if( $res['insert'] == "true"){
                    if($res['auth'] == "true"){
                        $result = $res['result'];
