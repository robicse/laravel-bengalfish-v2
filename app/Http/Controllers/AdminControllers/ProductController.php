...
class ProductController extends Controller
{
  ...
  public function addinventory(Request $request){
    ...
    public function ajax_get_products($id){
        $title = array('pageTitle' => Lang::get("labels.ProductInventory"));
        $result = $this->products->ajax_get_products($id);
        $opt = '<option>select one</option>';
        if(!empty($result)){
            foreach($result as $value){
                $opt .= '<option value="'.$value->products_id.'">'.$value->products_name.'</option>';
            }
        }
        return  $opt;
  ...
  public function addinventoryfromsidebar(Request $request){
      $title = array('pageTitle' => Lang::get("labels.ProductInventory"));
      $language_id = '1';
      $category_list = $this->category->category_list($language_id);
      //dd($categorry_list);
      $result = $this->products->addinventoryfromsidebar();
      return view("admin.products.inventory.add1", $title)->with('result', $result)->with('categories', $category_list);
