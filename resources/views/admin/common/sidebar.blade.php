...
<span>      {{ trans('labels.link_news') }}</span> <i class="fa fa-angle-left pull-right"></i>
      ...
      $status_check = DB::table('reviews')->where('reviews_read',0)->first();
        if($check->products_view == 1){
      ?>
        <li class="treeview {{ Request::is('admin/reviews/display') ? 'active' : '' }}">
          <a href="{{ URL::to('admin/reviews/display')}}">
            <i class="fa fa-users" aria-hidden="true"></i> <span>{{ trans('labels.reviews') }}</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;@if($status_check)<span class="label label-success">{{ trans('labels.new') }}</span>@endif
          </a>
        </li>
      <?php } ?>
      <?php
